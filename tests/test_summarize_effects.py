...
def test_summarize_effects_quantiles():
    ...
    for y in summ:
        assert (y.min == y.quantiles["0.0"]).all()
        assert numpy.allclose(y.median, y.quantiles["0.5"])
        assert (y.max == y.quantiles["1.0"]).all()
...
def test_summarize_effects_empty():
    ...
    for y in summ:
        assert y.min is None
        assert y.mean is None
        assert y.median is None
        assert y.max is None
        assert y.min_rank is None
